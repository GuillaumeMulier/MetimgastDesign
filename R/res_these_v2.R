...
# Créé le 28/09/2021, modifié le 01/02/2022                     #
# ------------------------------------------------------------- #
...
source("R/realisations_essais_metimgast_v2.R", encoding = "utf-8")
...
tableau_results[, c("nb_pts_simon", "temps_etude_simon", "accept_ttt_simon", "arret_prema_simon",
                    "arret_fut_simon", "arret_tox_simon", "nb_eff_simon", 
                    "nb_tox_simon")] <- NA_real_
tableau_results[, c("analyse_med_topiva", "analyse_med_bopiva", "analyse_med_simon")] <- NA_character_
...
for (i in seq_len(nrow(liste_scenars))) {
  ...
  # Simon avec approche Ivanova donc observation complète à chaque analyse intermédiaire
  tableau_results[i, c("nb_pts_simon", "temps_etude_simon", 
                       "arret_fut_simon", "arret_tox_simon", "nb_eff_simon", 
...
rm(tableau_results, cohorte_2, essais, p_reel)
...
tab_probas[, c("rejet_topiva", "rejet_bopiva", "rejet_simon", "rejet_classique", "rejet_toxrapp", "rejet_toxprior")] <- NA_real_
for (i in seq_len(nrow(tab_probas))) {
  ...
  # TOP avec approche Ivanova au temps du TOP
  tab_probas$rejet_topiva[i] <- simu_topiva(
    ana_inter_eff = c(30, 51),
    ana_inter_tox = c(rep(5, 4), rep(10, 6), 1),
    liste_essais = essais,
    liste_cohortes = cohorte_2,
  ...
  # BOP avec approche Ivanova donc observation complète à chaque analyse intermédiaire
  tab_probas$rejet_bopiva[i] <- simu_bopiva(
    ana_inter_eff = c(30, 51),
    ana_inter_tox = c(rep(5, 4), rep(10, 6), 1),
    liste_essais = essais,
    liste_cohortes = cohorte_2,
  ...
  # Simon avec l'approche Ivanova
  tab_probas$rejet_simon[i] <- simu_simon(
    ana_inter_eff = c(30, 51),
    ana_inter_tox = c(rep(5, 4), rep(10, 6), 1),
    liste_essais = essais,
    liste_cohortes = cohorte_2,
...
save(tab_probas, file = paste0("data/probas_h0h1_", format(Sys.Date(), "%Y%m%d"), ".Rdata"))
...
tableau_results_Rvar[, c("nb_pts_bopiva", "temps_etude_bopiva", 
                         "accept_ttt_bopiva", "arret_prema_bopiva", "arret_fut_bopiva", "arret_tox_bopiva", "nb_eff_bopiva", 
                         "nb_tox_bopiva")] <- NA_real_
tableau_results_Rvar[, c("nb_pts_simon", "temps_etude_simon", 
                         "accept_ttt_simon", "arret_prema_simon", "arret_fut_simon", "arret_tox_simon", "nb_eff_simon", 
                         "nb_tox_simon")] <- NA_real_
...
rm(tableau_results_Rvar, resultats_toxprior, resultats_toprapp, resultats_topclas, cohorte_2, essais, p_reel)
...
save(tab_probas, file = paste0("data/probas_h0h1_Rind_", format(Sys.Date(), "%Y%m%d"), ".Rdata"))
